    ...
    <div>
        <div class="page-header">
            ...
            <div class="d-flex align-center gap-2">
                <v-btn variant="outlined" color="success" prepend-icon="mdi-file-excel" :loading="downloadingTemplate"
                    :disabled="downloadingTemplate" @click="downloadImportTemplate">
                    Product Template
                </v-btn>
        ...
        <v-card class="mb-4">
            <v-card-text>
                <v-row>
                    ...
                    <v-col cols="12" md="2">
                        <v-select v-model="categoryFilter" :items="categoryFilterOptions" label="Filter by Category"
                            prepend-inner-icon="mdi-folder" variant="outlined" density="compact" clearable
                            @update:model-value="loadProducts" :loading="loadingCategories"></v-select>
                    </v-col>
                    <v-col cols="12" md="2">
                ...
                <v-table>
                    <thead>
                        <tr>
                            ...
                            <th class="sortable" @click="onSort('id')">
                                <div class="d-flex align-center">
                                    Image
                                    <v-icon :icon="getSortIcon('id')" size="small" class="ml-1"></v-icon>
                                </div>
                            </th>
                    ...
                    <tbody>
                        ...
                        <tr v-if="loading" v-for="n in 5" :key="`skeleton-${n}`">
                            ...
                            <td>
                                <v-skeleton-loader type="chip" width="50" height="24"></v-skeleton-loader>
                            </td>
                            <td>
        ...
        <ProductFormDialog v-model="showDialog" :editing-product="editingProduct" :loading="loadingProduct"
            ...
            :gallery-error="galleryError" :form-errors="formErrors" @update:form="form = $event"
            ...
            @update:image-url-inputs="imageUrlInputs = $event" @update:thumbnail-error="thumbnailError = $event"
            @update:gallery-error="galleryError = $event" @close="closeDialog" @save="saveProduct"
...
import ProductDetailsDialog from './dialog/ProductDetailsDialog.vue';
import ProductFormDialog from './dialog/ProductFormDialog.vue';
...
export default {
    ...
    data() {
        return {
            ...
            featuredFilter: null,
            // Override mixin's default sortBy to match controller default
            sortBy: 'order',
            sortDirection: 'asc',
            publishedOptions: [
    ...
    methods: {
        async downloadImportTemplate() {
            try {
                ...
                const response = await this.$axios.get('/api/v1/products/import-template', {
                ...
                this.handleApiError(error, 'Failed to download import template');
            } finally {
        ...
        async openDialog(product = null) {
            ...
            // Clear form errors when opening dialog
            this.formErrors = {};
        },
        ...
        async saveProduct() {
            try {
                ...
                const formData = {
                    ...
                    order: this.form.order !== null && this.form.order !== undefined ? parseInt(this.form.order) : 0,
                ...
                // Extract validation errors if it's a 422 validation error
                if (error.response?.status === 422 && error.response?.data?.errors) {
                    ...
                    // For other errors, clear form errors and show general message
                    this.formErrors = {};
                    const errorMessage = error.response?.data?.message ||
                        error.response?.data?.error ||
                        error.message ||
        ...
        applyProductDataToForm(data) {
            ...
            this.form = {
                ...
                order: data.order !== null && data.order !== undefined ? data.order : 0,
